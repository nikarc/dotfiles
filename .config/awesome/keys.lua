...
keys.globalkeys = gears.table.join(
  ...
  awful.key({ superkey }, "plus", function() awful.tag.incgap(5, nil) end,
    { description = "increment gap size for the current tag", group = "gaps" }),
  ...
  -- Prompt
  -- awful.key({ superkey, ctrlkey }, "r", function () awful.screen.focused().mypromptbox:run() end,
  --         {description = "run prompt", group = "launcher"}),
  awful.key({ superkey, ctrlkey }, "d", apps.discord),
  -- function()
  ...
  -- Layout
  awful.key({ superkey }, "v", beautiful.layout_fairv,
    { description = "FairV layout", group = "layout" }),
  awful.key({ superkey }, "x", beautiful.layout_fairh,
    { description = "FairH layout", group = "layout" })
)
...
-- Move to edge or swap by direction
  ...
  --   awful.spwan(user.terminal .. " -e \"node\"")
  -- end),
  ...
  -- Resize vertically
  awful.key({ superkey, altkey }, "k", function() awful.client.incwfact(0.01) end),
  awful.key({ superkey, altkey }, "j", function() awful.client.incwfact(-0.01) end),

  awful.key({ superkey }, "o", function(c) c:move_to_screen() end,
    { description = "Move to screen", group = "client" }),
  awful.key({ superkey }, "i", function(c) c:move_to_screen(c.screen.index - 1) end,
    { description = "Move to screen", group = "client" })
)
...
-- (Double tap - Toggle maximize) -- A little BUGGY
  ...
  awful.button({}, 9, function()
    local c = mouse.object_under_pointer()
    client.focus = c
    --awful.placement.centered(c,{honor_padding = tnue, honor_workarea=true})
    c.floating = not c.floating
  end),
  -- Side button down - toggle ontop
  awful.button({}, 8, function()
